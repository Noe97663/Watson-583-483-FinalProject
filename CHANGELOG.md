...
## 2026-05-03 — Polish pass: significance, IDF analysis, prompt snippets

### `src/significance.py` (new)
- McNemar exact-binomial paired test over per-question correctness for
  every adjacent mode pair (baseline/improved/llm).
- Headline finding: **baseline→improved p = 0.227 (NOT significant)**;
  **improved→llm p = 1.9e-9** and **baseline→llm p = 5.5e-10** (both
  highly significant).

### `src/idf_analysis.py` (new)
- Computes per-clue summary IDF (mean, median, or max) against the
  index's body postings; bins into equal-frequency quintiles; reports
  P@1 per bin and saves a matplotlib chart to
  `results/idf_histogram.png`.
- Headline finding: max-IDF shows a **non-monotone "sweet spot"
  pattern** — P@1 is 10% in the lowest IDF bin, peaks at 45% in the
  middle bin, drops to 0% in the ultra-rare top bin. The original
  "rare = easy" claim is refined: BM25 wants *discriminating*, not
  just *rare*, tokens.

### `report.tex`
- Added `graphicx` for the IDF figure.
- Q1: footnoted the variant table to mark the `0.12`/`0.15` rows as
  ablated prototype runs (not reproducible from current source).
- Q2: added subsection on the LLM-bootstrapped `instructions.md`
  (the natural-language brief that drove every later session was
  itself first generated by an LLM from `project.md`); added two
  verbatim prompt snippets — the reranker's full system prompt and
  the regression-diagnosis prompt.
- Q3: new "Statistical Significance (McNemar's Exact Test)"
  subsection with contingency-table results and the honest reading
  that the rule-based gain is not significant at n=100.
- Q3: explicit acknowledgment that the B-sweep used the same 100
  questions as the headline metrics.
- Q4: new "Why the Easy 25 Work — Clue-IDF Analysis" subsection
  with the IDF-histogram figure and the "sweet spot" finding.
- Compiles cleanly to 17 pages.

### `README.md`
- Mirrored the report changes: McNemar significance table, IDF
  histogram findings, B-sweep eval-set acknowledgment.
- Repository layout updated with the two new analysis scripts.
